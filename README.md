...
```
A tab-delimited file (`promoter_delineations.tsv`) with promoter delineation points in terms of distance from the TSS (columns 1 (start) and 2 (end)) and an “authenticity” score denoting the likelihood of the delineation to be the actual promoter (column 3) expected as output. The most optimal promoter delineation is listed on the bottom line of output (i.e. Check by `tail -1 promoter_delineations.tsv`). 1 hr. runtime expected.
## Instructions for use
